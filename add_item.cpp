...
void add_item::CheckState(QListWidgetItem *changedItem)
{
...
void add_item::AddShopsToListWidget(const QStringList &shops)
{
    ui->listWidget->clear();
    ...
    connect(ui->listWidget, &QListWidget::itemChanged, this, &add_item::CheckState);
}


void add_item::AddSectionsToComboBox(const QStringList &section)
{
...
void add_item::AddPublisherToComboBox(const QStringList &pub)
{
...
QString add_item::GetSelectedShopNumber()
{
    for (int i = 0; i < ui->listWidget->count(); i++) {
        QListWidgetItem *item = ui->listWidget->item(i);
...
void add_item::on_pushButton_2_clicked()
{
    ...
    QString line =  "addbooks|" + shopnumber + "|" + section + "|" + authorName + "|" + authorFamilia + "|" + authorOtchestvo
